...
sql_str  :=  'INSERT ALL INTO LINE_ITEMS (order_id , weight, volume, hazard, product ) VALUES (:order_id_val, 1000.1, 1.0, ''Y'', ''petrol'') INTO LINE_ITEMS (order_id , weight, volume, hazard, product ) VALUES (:order_id_val, 2000.0, 2.0, ''N'', ''water'') SELECT 1 FROM dual';
   select ADDRESS_OBJ('COLORADAO SPRINGS', 'CO', '80801') into l_from_address  from dual;
   select ADDRESS_OBJ('Honolulu', 'HI', '96621') into l_to_address  from dual;
  ...
  VENU.ORDER_PROCESS_PACKAGE.SAVE_ORDER_SP(l_from_address, l_to_address, l_col_data, 'instructions', order_id, message);
